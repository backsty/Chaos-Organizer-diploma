...
export default class Sidebar {
  ...
  // --- Исправленный поиск с поддержкой русского и английского, регистронезависимый ---
  validateSearchInput() {
  ...
  // --- Подсветка с учетом регистра и языка ---
  previewMessageMarkup(data) {
    let replace = data.message;
    if (this.searchInputValue) {
      // Экранируем спецсимволы для RegExp
      const escaped = this.searchInputValue.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      // Флаг "i" для регистронезависимого поиска, "u" для поддержки юникода (русский)
      const regex = new RegExp(escaped, 'giu');
      replace = data.message.replace(regex, (value) => `<span class="marker">${value}</span>`);
    }
  ...
  async getRequest(value) {
    ...
    if (result.success) {
      // Фильтрация по регистронезависимому совпадению на клиенте (на случай, если сервер не фильтрует)
