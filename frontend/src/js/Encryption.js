...
export default class Encryption {
  ...
  encryptMessage(word, key) {
    const encrypted = CryptoJS.AES.encrypt(JSON.stringify(word), key).toString();
    return `ENCRYPTED:${encrypted}`;
  }

  decryptMessage(word, key) {
    try {
      if (!word.startsWith('ENCRYPTED:')) return word; // Не шифрованное сообщение
      const encrypted = word.replace('ENCRYPTED:', '');
